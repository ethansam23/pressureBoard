...
```
python soak_verify.py --capture logs/<run> \
       --reference ../host_tests/refs/phaseA_rate1000.csv --rate 1000
```

The reference streams are committed in `host_tests/refs/` — generated by the
same `sim_profile_code()` the firmware runs, so the bench PC needs no C
toolchain. `make -C host_tests` regenerates and diffs them, failing on drift.
...
against `host_tests/refs/phaseB_rate1000.csv`.
...
- [ ] Verify with `host_tests/refs/phaseB_rate100.csv` with `--rate 100`
...
- [ ] `RESULT: PASS` against `host_tests/refs/full_rate1000.csv`
